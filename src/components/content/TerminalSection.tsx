  ...
  const processCommand = (command: string) => {
    ...
    setTimeout(() => {
      ...
      if (command.trim().toLowerCase() === 'skills --list' || command.trim().toLowerCase() === 'skills' || command.trim().toLowerCase() === 'ls skills') {
        ...
        setOutput(
          <p className="text-red-500 animate-fadeIn">Command not found: {command}</p>
        );
      }
    }, 600);
  };
      ...
      {!isMinimized && (
        ...
        >
          ...
          <div className="mt-4 flex items-center group">
            ...
            />
            <span className={`w-2.5 h-5 bg-green-500 ml-0.5 ${cursorVisible ? 'opacity-100' : 'opacity-0'} transition-opacity duration-100 shadow-[0_0_5px_rgba(0,255,0,0.7)]`}></span>
          </div>
        </div>
      )}
    </div>
