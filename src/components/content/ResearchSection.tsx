...
const ResearchSection: React.FC<ResearchSectionProps> = ({ data }) => {
  ...
  return (
    <div className="animate-fadeIn my-8 sm:my-12 w-full">
      <h2 className="text-xl sm:text-2xl font-semibold text-white mb-6 sm:mb-8 flex items-center border-b border-gray-700 pb-2 sm:pb-3">
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-1.5 sm:p-2 rounded-lg shadow-lg mr-2 sm:mr-3">
      ...
      <div className="grid grid-cols-1 gap-5 sm:gap-6 md:gap-8 w-full">
        {data.map((paper, index) => (
          <div 
            key={index} 
            className={`relative group transition-all duration-500 ${
              hoveredIndex === index ? 'scale-[1.01] sm:scale-[1.02]' : 'scale-100'
          ...
          >
            ...
            <div className="relative bg-gradient-to-br from-[#1a1a1a] to-[#252525] border border-gray-800 p-4 sm:p-6 rounded-xl group-hover:border-emerald-500/40 transition-all duration-300 shadow-xl hover:shadow-emerald-900/20 overflow-hidden">
              {/* Decorative elements */}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                <div className="bg-gradient-to-br from-[#1e3a30] to-[#2d4a40] p-2 sm:p-3 rounded-lg shadow-inner inline-flex">
                  <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400" />
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-teal-200 leading-tight">{paper.title}</h3>
              </div>
              ...
              <div className="mt-4 sm:mt-5 flex flex-wrap justify-between items-center gap-3">
                <button 
                  className="text-xs sm:text-sm text-gray-400 hover:text-emerald-300 transition-colors flex items-center gap-1"
                ...
                <div className="flex gap-2 sm:gap-3">
                  <a 
                    ...
                    className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm transition-all hover:shadow-lg hover:shadow-emerald-500/20 flex items-center gap-1.5 group/btn"
                  >
              ...
              {/* Progress indicator */}
              <div className="w-full h-1 bg-gray-800 rounded-full mt-4 sm:mt-6 overflow-hidden">
                <div 
